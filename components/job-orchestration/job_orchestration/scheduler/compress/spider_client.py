...
def submit_job(db_conn, task_params) -> Optional[uuid.UUID]:
    ...
    try:
        ...
        db_cursor.executemany(_task_insert_input_value_query, [(task_ids[i], 2, _int_list_typename, msgpack.packb(task_param["tag_ids"] if task_param["tags_id"] is not None else [])) for i, task_param in enumerate(task_params)])
