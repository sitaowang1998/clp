...
from spider_orchestration.scheduler.scheduler_data import (
    CompressionJob,
...
def get_results(spider_job):
    if spider_job.get_status() == SpiderJobStatus.Succeeded:
        return spider_job.get_results()
    else:
        return None
...
def poll_running_jobs(db_conn, db_cursor):
    ...
    for job_id, job in scheduled_jobs.items():
        ...
        try:
            returned_results = get_results(job.spider_job)
...
def main(argv):
    ...
    try:
        spider_driver = SpiderDriver(clp_config.database.get_spider_url())
    except Exception as err:
        logger.exception(f"Failed to initialize SpiderDriver: {err}")
        return -1
